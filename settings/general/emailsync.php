...
if (isset($_POST['settings'],$_POST['type'],$_POST['action']) && $_POST['type'] == $settings['key']) {
	...
	if (!isset($_POST['handled']) && $emailsync['action'] == 'load') {
		...
		if (!empty($emailsync['job'])) {
			...
			if (!empty($emailsync['job']['last_run'])) $emailsync['status_items'][] = ['id'=>$settings['key'].'-'.$emailsync['job']['id'].'-last-run','description'=>language__get($user['language'],'_emailsync_last_run'),'subtitle'=>format__date_relative((int) $emailsync['job']['last_run'],'relative',$user['language'],true)];
			...
			$emailsync['job_tabs'] = create__tablist($settings['key'].'-'.$emailsync['job']['id'].'-tabs',[
				...
				'status'=>[create__list($settings['key'].'-'.$emailsync['job']['id'].'-status',$emailsync['status_items'],['clear'=>true])],
...
foreach (\emailsync\Statistics::daily($emailsync['jobs']) as $emailsync['day'] => $emailsync['messages']) $emailsync['statistics_graph']['points'][] = ['label'=>format__date_relative($emailsync['day'],'date',$user['language']),'data'=>['messages_synchronized'=>$emailsync['messages']]];
